 ...
 */
/**
 * 그리고 지금은 일곱이다 (POLISHED_REFERENCE_PARITY_DESIGN.md §4.3 · R2).
 *
 * 위 주석이 `[4]` 하나로 좁힌 과정을 적어뒀는데, 레퍼런스 정합이 그것을 다시
 * 편다 — 목업은 3(선택 바) · 5(기한·키캡) · 6(행·nav·팝오버) · 8(아이콘 버튼)
 * · 9(레일) · 12(떠 있는 면)를 쓴다.
 *
 * 4가 아직 여기 있는 이유는 R2다. 토큰 레이어가 `--radius-*`를 전역으로 6px에
 * 걸어서 대부분의 화면은 이미 레퍼런스 값으로 그려지지만, 리터럴로 `4px`을 박아둔
 * 다섯 자리가 레거시 파일에 남아 있다. 이 스펙은 선언이 아니라 화면을 재므로,
 * 그 다섯이 옮겨가기 전에 4를 빼면 잘못된 것을 잡는 게 아니라 순서를 잡는
 * 실패가 된다 — 위 문단이 이행 중에 `[4, 6, 8, 10, 12]`를 유지한 것과 같은 이유다.
 *
 * 레거시 19개 파일이 레퍼런스 자로 옮겨가면 4를 뺀다.
 */
const SCALE = [3, 4, 5, 6, 8, 9, 12];
