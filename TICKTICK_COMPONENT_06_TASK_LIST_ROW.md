...
## 16. 완료된 작업의 눈금과, 체크박스 하나의 크기 (2026-09-03)

요구 셋: 완료된 작업의 **체크 표시도 회색**으로 · 제목의 **취소선 제거** · 체크박스를
**참조 앱 비율로 축소**, 전역으로.

### 16.1 [실측] 우리 체크박스는 실제로 컸다 — 두 배로
...
|---|---|---|
| 아이콘 박스 | 17 | 17 |
| **실제로 그려지는 것** | **14.9 × 14.9 글리프**(SVG) | **17 × 17** — 네이티브 `<input>`은 위젯 전체가 그려진다 |
| 터치(coarse) | (해당 없음) | **22 × 22** |
| 제목 | 14 / lh 24 | 14 |

핵심은 **"아이콘 박스"와 "그려지는 글리프"가 다르다**는 것이다. 참조 앱은 17px 슬롯 안에
14.9px 눈금을 그리고, 우리는 17px 슬롯을 17px 위젯으로 꽉 채웠다. 같은 숫자를 베꼈는데
화면에서는 15% 더 컸고, `pointer: coarse`가 맞는 기기(터치 되는 노트북 포함)에서는
**22px — 제목의 1.57배**가 됐다. "내 체크박스는 너무 크다"는 관찰은 정확했고, 두 번
정확했다.

### 16.2 결정: 숫자 하나 — `--check-size: 15px`

앱에는 체크박스 크기가 **여섯 개** 있었다.

| 자리 | 전 | 후 |
|---|---|---|
| 공유 작업 행 `.tm-task-check input` | 17 (coarse 22) | `var(--check-size)` |
| Today의 사각 버튼 `.tdy-check` | 22 | `var(--check-size)` |
| kit의 원형 `.ff-check` | 22 | `var(--check-size)` |
| Today 분류 `.tdy-triage-check` | 16 | `var(--check-size)` |
| Detail의 완료 `.tm-drawer-done input` | 16 | `var(--check-size)` |
| Space Detail `.sdv-check-row input` | 16 | `var(--check-size)` |

**15는 참조 앱이 실제로 그리는 14.9다.** 여섯 화면에서 같은 뜻인 컨트롤이 여섯 크기일
이유가 없고, 토큰이 하나면 다음에 조정할 때도 하나다.

### 16.3 터치 목표는 그림이 아니라 라벨이 진다

`pointer: coarse`에서 박스를 22로 키우던 규칙을 **지웠다.** 44px 목표는 그림이 아니라
`.tm-task-check` 라벨이 지면 된다 — 이미 행 전체 높이로 늘어나 있으므로, 좌우
padding을 14로 주면 **43 × 44**가 된다. 참조 앱이 하는 것이 정확히 이것이다(§6: 히트
영역 17 × 40, 글리프 14.9).

시험도 그렇게 고쳤다: 세로만 재던 것이 이제 **가로도** 잰다. 박스가 안 자라기 때문에
한 방향만 재면 통과하면서 손가락은 빗나가는 상태가 가능해졌기 때문이다.

### 16.4 완료 = 회색, 그리고 줄은 없앤다

**회색:** `.tm-task-check input:checked { accent-color: var(--text-tertiary) }`.
클래스가 필요 없다 — **체크된 상자는 곧 완료된 작업**이므로 상태가 이미 DOM에 있다.
액센트는 아직 할 일의 색이고, 끝난 줄을 그 색으로 칠하면 남은 일의 목록에서 완료된
행이 제일 밝아진다. Today의 사각 버튼과 kit의 원형은 `--success`(초록)였고, 같은 이유로
회색이 됐다.

**취소선:** 작업 제목에서 전부 뺐다 — 목록(`tm-task-title`) · Today(`tdy-row`) ·
보드 카드 · 리스트 뷰 · 캘린더 블록과 칩 · Drawer의 하위 작업 · Space Detail의 하위 작업.
회색과 체크된 상자가 이미 같은 말을 하고 있고, 취소선은 그 셋 중 **읽기를 방해하는**
하나다.

**한 곳만 남겼다: 체크리스트 항목**(`.tm-checklist-items li.is-checked`). 체크리스트
항목은 작업이 아니라 작업 *안의 한 줄*이고, 줄을 긋는 것은 종이 목록이 하는 일이다.

### 16.5 검증

- 유닛 **2,390개 통과** · `tsc` 통과 · E2E 전체 통과
- 실제 앱(dev, ko, 다크): 완료 행이 **회색 체크 + 회색 제목 + 취소선 없음**,
  미완료 행은 그대로. 박스 15px, 제목 14px
- 터치 에뮬레이션(375×812): `pointer: coarse` 참, 박스 **15**, 라벨 목표 **43 × 44**,
  행 45
