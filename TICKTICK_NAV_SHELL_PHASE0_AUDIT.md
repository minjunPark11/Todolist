...
## P0-2 종결 — Today는 하나다 (2026-09-03)

§12.129의 표가 "둘 중 무엇이 살아남는지는 P0-2의 질문"이라고 열어 둔 자리다. **답:
Tasks Module의 `/today` Scope가 남고, `/app`의 Today 페이지는 지운다.**

### 무엇이 문제였나 [실측]

사용자가 스크린샷을 보내며 "이 화면은 뭐지?"라고 물었다. **그 질문 자체가 답이다** —
자기 앱의 화면을 못 알아본 이유는 같은 이름의 화면이 둘이고 주소만 갈라져 있었기
때문이다.

- 사이드바는 `다음 7일`을 hover로 밝히고 있는데 본문 제목은 `오늘`이었다. 이 페이지는
  사이드바에 `current={null}`을 넘기므로 **어느 행도 자기가 있는 곳을 표시하지 않는다.**
- Rail의 `작업` 버튼은 `/today`(Module)로 간다. `/app`은 Rail에 없다 — **부팅 기본값과
  옛 주소로만 도달하는 화면**이었다.
- `defaultView` 설정이 `/today`인 계정은 `pathForDefaultView`의 `default:` 때문에
  **`/app`(페이지)으로 열렸다.** 설정에 적힌 주소와 실제로 열리는 화면이 달랐다.

### 같이 나간 것

| 것 | 상태 |
|---|---|
| `받은함 정리`(InboxTriage) | **삭제.** 일괄 분류 서랍 |
| 오늘 계획 버킷(FocusQueue) | **삭제.** `setTodayBuckets`가 유일한 쓰기였다 |
| TimeRail · InlineCapture · QuickAddTaskModal | 삭제 |
| `todayIntent`(triage/quickAdd) · 단축키 `n` | 삭제. `i`는 `/inbox` Scope로 간다 |
| `settings.showCompletedTasks` 토글 | 삭제. 읽던 화면이 그것뿐이었다 |
| `04-today.css` 1,035줄 + 다른 3개 시트의 `tdy-` 규칙 | 삭제 |
| 문자열 **81개** | 삭제(카탈로그 고아 0 유지) |

**기록은 안 지운다.** `dailyPlans`는 그대로 있고, `planTaskForDay`가 계속 쓰고,
`hasTodayPlan`이 Module의 Today Scope에서 계속 읽고, 서버의 `todayTasks` 질의가 계속
버킷으로 나눈다. 사라진 것은 **버킷을 손으로 재배열하는 화면** 하나다.

### 주소는 하나도 안 죽는다

`/app`은 `RETIRED_ROUTES`에 들어가 `/today`로 **이동한다**. `/archive` · `/spaces` ·
`/projects` · `/goals`도 같이 그쪽을 가리키게 바뀌었다.

그리고 그 넷의 동작이 바뀌었다: 전에는 "이 주소는 페이지를 가리킨다(`namesAPage` = true)"
라서 **주소는 `/archive`인데 화면은 Today**였다. 이제는 셋 다 진짜 리다이렉트다 —
주소창과 화면이 어긋나지 않는다.

`pageForPath`는 `PageId | null`을 답한다. 네 페이지 중 하나가 아니면 **null**이고, 그건
"Tasks Module의 주소"라는 뜻이다. 예전의 `?? "today"` 폴백은 **모르는 주소마다 Today
페이지를 그리던** 것이었다.

### 검증

- 유닛 **2,375개 통과** · `tsc` 통과
- 삭제: 컴포넌트 6개(2,140줄) · 스타일시트 1개(1,035줄) · 문자열 81개
