...
- **읽는 순서:** R.1.1 채택 결정 → R.6 확정 결정(D-01~D-28) → R.8 구현 순서. 설계서 본문은 필요할 때만 펼친다.
...
**P0는 끝났다.** 다음은 이 브랜치를 main으로 보내는 일이고, 그 전에 알아야 할 것 셋:

1. **설계서 §2.14는 개정 대상이다** (D-25). Search는 오버레이가 아니라 페이지고, Ctrl/Cmd+K가 별개의 메뉴다. 다음 개정판과 대조할 때 이 줄부터 볼 것.
