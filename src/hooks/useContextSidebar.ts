...
//
// Owned by the App Shell, per §3.66: the width is shell state, not something
// either inner shell keeps for itself. That is the whole point of P0-3 —
// before this, neither shell had a width at all.
//
// A collapsed flag lived here too, until the button that set it was removed
// (AppShell). Nothing else could reach the state, so a sidebar could only get
// stuck collapsed by a stored "1" nobody could clear.
//
...
function persist(key: string, value: string): void {
  try {
    ...
    // A session without storage still resizes; it just forgets.
  }
}
