...
import { archiveCustomStatus, moveGoalToStatus, patchCustomStatus, reconcileGoalStatuses, sanitizeCustomStatuses } from "./customStatuses";
...
describe("board lists", () => {
  it("sanitizes, deduplicates, and reindexes lists", () => {
    expect(sanitizeCustomStatuses([{ id: "a", name: " A ", order: 8 }, { id: "a", name: "dup", order: 0 }, { id: "b", name: "B", order: 2 }]))
      .toEqual([{ id: "b", name: "B", order: 0 }, { id: "a", name: "A", order: 1 }]);
  });
  it("clears invalid cross-board list assignments", () => {
    expect(reconcileGoalStatuses([project], [goal({ boardListId: "missing", boardOrder: 2 })])[0]).toMatchObject({ boardListId: undefined, boardOrder: undefined });
  });
  it("keeps a goal but clears its board fields when the project is gone", () => {
    expect(reconcileGoalStatuses([], [goal({ boardListId: "l1", boardOrder: 2 })])[0]).toMatchObject({ projectId: undefined, boardListId: undefined, boardOrder: undefined });
  });
  it("moves a goal only to a valid list and archives without deleting goals", () => {
    const moved = moveGoalToStatus([goal()], [project], "g1", "l1", "now");
    expect(moved[0].boardListId).toBe("l1");
    const archived = archiveCustomStatus([project], moved, "p1", "l1", "later");
  ...
  it("reorders lists and persists contiguous order values", () => {
    ...
    expect(patchCustomStatus([withLists], "p1", "b", { order: 0 }, "now")[0].boardLists?.map((status) => [status.id, status.order]))
      .toEqual([["b", 0], ["a", 1]]);
  });
});
