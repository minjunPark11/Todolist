...
### 10.10 상자 안에 상자가 있었다 (2026-09-03)

**[실측]** 배포 뒤 실제 앱에서 잡혔다. 줄은 하나인데 그 안에 **두 번째 상자**가 그려지고
있었다 — `+` 옆에 이음매가 보이고, 포커스를 주면 테두리가 두 겹이었다.

| | 바깥(`.tm-quickadd-box`) | 안(`.tm-quickadd-title`) |
|---|---|---|
| 배경 | `rgba(255,255,255,0.06)` | **`rgb(28,28,30)`** |
| 보더 | 1px transparent | **1px `rgba(255,255,255,0.08)`** |
| radius | 10 | **8** |
| padding | 0 10px | **0 12px** |

원인은 `21-components.css`의 **필드 규칙**이다. 그 규칙은 다섯 갈래였던 입력 언어를 하나로
모은 것이고, `.tm-quickadd-title`이 **그 목록에 이름으로 올라가 있었다.** 게다가 목록에서
이름을 빼도 소용이 없다 — 같은 규칙의
`input:not([type="checkbox"]):not([type="radio"]):not([type="range"])`가 **모든 `input`을**
잡고, 그 특정도가 (0,3,1)이라 다른 파일의 `.tm-quickadd-title`(0,1,0)은 이길 수 없다.
이 파일 자신이 Detail 제목에 대해 적어 둔 함정 그대로다.

**고침: 규칙이 이 입력을 주장하지 않게 한다** — `:not(.tm-quickadd-title)`을 붙였다.
싸워서 이기는 것이 아니라 대상에서 빼는 쪽이고, 예외를 **규칙이 쓰인 자리에 적는다**는
그 파일의 원칙 그대로다. 퀵애드의 입력은 더 이상 필드가 아니다: §10이 만든 것은 채워진
**줄**이고, 그 안의 `input`은 글자일 뿐이다.

같이 사라진 두 번째 피해: 필드 규칙의 `min-height: var(--density-tap)`. **터치 기기에서는
32px 줄 안의 입력이 44px 바닥을 갖고 있었다.**

E2E의 `componentLanguage` 필드 가족에서도 뺐다 — 일부러 갖지 않기로 한 모양을 두고
"필드와 같은가"를 묻는 시험이기 때문이다.
