...
**Playwright가 살 것은 세 번째 줄뿐이다.** 다만 그 줄이 작지 않다 — 400% 줌에서 Board 버튼이 잘려 **접근 불가**였던 것과 80자 이름에 모달이 늘어나는지는 레이아웃이 없으면 볼 수 없고, **사람이 매번 다시 걸어주지 않으면 조용히 회귀하는 종류**다.

### 결정: 27개 전부, 세 뷰포트에서

`playwright.config.ts` + `e2e/addList.spec.ts`. **27개가 파일에 전부 열거된다** — 게이트의 숫자와 파일의 숫자가 같아야 차이가 눈에 보인다.

**Chromium 하나만 쓴다.** §17.3이 요구하는 Desktop / Tablet / Mobile은 **엔진 셋이 아니라 뷰포트 셋**이다. 같은 단언을 세 렌더링 엔진에 통과시키면 아무도 요청하지 않은 벤더 CSS 커버리지를 CI 시간 3배로 사는 셈이다.

**5개는 이 앱에 없는 서버를 전제하므로 `skip`이고, 이유를 테스트 본문에 적어 남긴다** — 조용히 빼면 27이 22가 된 것을 아무도 모른다.

| 시나리오 | 왜 |
|---|---|
| E2E-19 권한 변경 | 공유·권한 모델이 없다. 나중에 협업이 생기면 **다시 발견할 게 아니라 이 자리에 쓰면 된다** |
| E2E-21 Sidebar cache 실패 | 캐시 계층이 없다. 사이드바 개수는 화면이 돌리는 그 쿼리에서 나온다(§12.14, Gate 2) — 어긋날 두 번째 사본이 없다 |
| E2E-14·15·20의 문자 그대로의 형태 | 생성 시점에 네트워크 요청이 **없다**. 대신 **도달 가능한 쪽**을 검증한다: 오프라인에서 생성이 성공하고(§17.17이 요구한 복구 가능성보다 강하다), 리로드해도 같은 id 하나뿐이며, 라우팅과 무관하게 레코드가 남는다 |

**결과: 46 passed / 35 skipped.** (skip 대부분은 뷰포트 배분 — 데스크톱 시나리오는 모바일에서, 모바일 시나리오는 데스크톱에서 돈다.)

**CI에 넣었다.** §17.3이 릴리스 조건이라고 했으므로, 누가 기억해서 돌리는 것이 아니라 게이트여야 한다.

**E2E를 쓰면서 실제로 드러난 것 둘.** `addInitScript`는 **모든 문서 로드마다** 돌아서, 계정을 무조건 쓰면 리로드·`goto` 때마다 시작 상태로 되돌린다 — 그리고 리로드하는 스펙이 바로 "만든 것이 살아남는가"를 묻는 스펙이다. 그리고 데스크톱 아래에서는 사이드바가 off-canvas라(§15.14) 트리를 먼저 화면에 올려야 그 안의 무엇도 누를 수 없다. **둘 다 앱의 버그가 아니라 하니스가 앱을 실제로 쓰지 않으면 몰랐을 사실이다.**
