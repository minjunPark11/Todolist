...
| 단계 | 무엇 | 왜 이 순서 |
|---|---|---|
| **S1** | 설정 필드 병합 (§4) | 순수 함수 하나. **S3보다 먼저여야 한다** — pull이 잦아지면 1.2의 손실도 그만큼 잦아진다 |
| **S2** | 기기별 커서 (§5) | 마이그레이션 + `googleCalendarSources`. 역시 S3보다 먼저: pull이 잦아진다고 구글 미러가 나아지지는 않는다 |
| **S3** | 자동 내려받기 (§3) | 앞의 둘이 고쳐진 뒤라야 "자주 읽기"가 순수한 이득이 된다 |
...
| 단계 | 테스트 |
|---|---|
| S1 | 서로 다른 필드를 고친 두 기기가 둘 다 살아남는다 · 같은 필드는 로컬이 이긴다 · baseline이 없으면 원격 그대로 |
| S2 | 커서를 기기별로 읽고 쓴다 · 다른 기기의 커서를 보지 않는다 · 커서 없는 기기의 첫 패스가 전체 나열이다 |
| S3 | 포커스가 pull을 부른다(쿨다운 안에서는 한 번만) · 우리 저장 직후의 realtime 이벤트는 무시한다 · pull 전에 큐를 비운다 |
