...
3. 앱 → 설정 → 캘린더 → **동기화 시간대** 에서 올바른 값을 고르고 확인한다. 그
   목록이 보여주는 값이 지금 고정돼 있는 시간대다 — 그것이 곧 진단이다. 고르면
   연결과 계정의 시간대가 같이 바뀐다. 둘은 같아야 한다: 이 앱이 저장하는 모든
   날짜와 시각은 시간대가 붙지 않은 벽시계 문자열이라 올바른 값이 하나뿐이다.
   (구글을 다시 연결해도 같은 일이 일어난다. 연결 흐름의 끝이 같은
   `bind_verified_google_calendar` 이기 때문이다.)
4. 다음 동기화가 전체 재읽기다 (`sync_token` 이 비워진다). 어긋나 있던 작업들이
   교정되고, 가져온 뒤 손댄 것만 검토로 올라온다.

거절되면 카드가 사유를 문장으로 말한다. 응답에 실려 오는 이름은 이렇다:
