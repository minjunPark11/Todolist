...
3. 앱 → 설정 → 일반 → **시간대** 를 올바른 값으로 고른다. 이 값이 다음 바인딩에서
   `sync_timezone` 이 된다.
4. 앱 → 설정 → 캘린더. 고정된 시간대가 3번에서 고른 값과 다르면 카드가 그 사실을
   말하고 **[Align time zone]** 을 내놓는다. 그것을 누른다. 같으면 아무것도 나오지
   않는다 — 맞출 것이 없다는 뜻이다. (구글을 다시 연결해도 같은 일이 일어난다.
   연결 흐름의 끝이 같은 `bind_verified_google_calendar` 이기 때문이다.)
