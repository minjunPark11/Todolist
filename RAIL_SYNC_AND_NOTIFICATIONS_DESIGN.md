...
## 11. 버튼이 이름값을 하게 — 계정 말고 나머지까지

> 상태: **구현됨** · 2026-09-06 (사용자 결정)
> 요청: "레일 버튼이 외부 캘린더까지 맞추게 하는 쪽으로" → 이어서
> "설정 › 캘린더 '전체 새로고침', 설정 › 데이터 '지금 백업', '로컬 데이터 업로드'
> 이것들도 그냥 다 레일 버튼으로 할 수 있게"

### 11.1 무엇이 문제였나 [실측]

버튼의 이름은 "지금 동기화"인데 F1이 맞추는 것은 계정 하나였다. 앱이 실제로 맞추는
것은 넷이고, 나머지 셋은 설정 화면 안쪽에 흩어져 있었다.

| 무엇 | 어디에 있었나 | 무엇을 호출하나 |
|---|---|---|
| 계정 (올리고 내려받기) | 레일 버튼 | `syncNow` |
| 외부 ICS 구독 | 설정 › 캘린더 › "전체 새로고침" | `syncAllExternalCalendars` |
| 이주 대기 중인 로컬 데이터 | 설정 › 계정 › "로컬 데이터 업로드" | `uploadLocalDataToSupabase` |
| 로컬 백업 파일 | 설정 › 데이터 › "지금 백업" | `autoBackup.backupNow` |

특히 세 번째는 `migrationPreviewCount > 0`일 때만 **행 자체가 그려졌다**. 계정 탭에
걸어 들어간 사람만 그런 게 있다는 것을 알았다.

### 11.2 누른 뒤 무슨 일이 일어나는가

```
uploadLocalDataToSupabase()   // 이주할 게 없으면 no-op, 있으면 MERGE (교체 아님)
  → saveQueue.drain()        // 올리기
  → loadSupabaseData()       // 내려받기
syncAllExternalCalendars()    // 켜진 ICS 구독 전부 (병렬)
backupNow()                   // 단, 예정일 때만 — §11.4
```

이주 업로드가 **큐를 비우기 전**에 온다. 그것이 같은 저장 큐로 쓰기 때문이다 — 먼저
비우면 방금 쓴 것이 안 보내진 채 남는다.

### 11.3 F2를 다시 읽었다 — 침묵의 정의가 바뀌었다

F2는 "계정이 없으면 버튼을 그리지 않는다"였다. 근거는 **누른 사람에게 침묵으로 답하는
버튼은 없는 버튼보다 나쁘다**였고, 그 원칙은 그대로다. 바뀐 것은 무엇이 침묵이냐다:
ICS 구독은 계정 없이 로컬에 살아 있고 새로고침할 대상이다. 그래서 **로그인하지 않았어도
켜진 구독이 하나라도 있으면 그린다.**

판단은 `src/app/railSync.ts`의 순수 함수 하나로 뺐다. 실행은 주인이 셋이라(계정은
`usePlannerData`, 구독은 `App`의 상태, 백업은 `useAutoBackup`) `App.tsx`가 조립하지만,
"지금 어떤 얼굴이냐"는 값만 보면 되는 질문이라 테스트가 값만 본다.

상태는 **도는 중 > 실패 > 쉬는 중**. 도는 동안의 정직한 상태는 "진행 중"이고, 실패는
끝난 뒤에도 남아 있으므로 가려지지 않는다. 로그아웃 상태에서는 계정 상태를 읽지 않는다 —
고칠 수 없는 실패를 계속 보여주게 되므로.

**부작용 하나**: 구독 하나의 ICS 주소가 깨져 있으면 레일 아이콘이 계속 실패로 남는다.
이건 옳다 — 앱이 맞추는 것 중 하나가 실패하고 있는 게 사실이고, 어느 것인지는 §3.2가
이미 알림으로 기록한다.

### 11.4 백업만 조건을 달았다 — 그리고 그 이유

`backupStamp`는 초 단위라 **부를 때마다 새 파일**이 생기고, `platform.backups.write`는
`keep` 개를 남기고 나머지를 지운다. 무조건 걸면 오후에 열 번 누른 사람이 역사적인 사본을
전부 밀어내고 같은 한 시간짜리 열 개를 갖게 된다 — 백업이 있으라고 있는 것의 반대다.

그래서 `isBackupDue`를 통과할 때만 쓴다. 그 함수는 한 시간짜리 타이머가 이미 쓰던
것이라 새 규칙이 아니고, "자동 백업 끄기"도 그대로 존중된다(꺼져 있으면 `false`).
레일 누르기의 뜻은 "지금 백업 파일을 하나 더"가 아니라 **"백업을 최신으로 유지"**다.

무조건 쓰기를 원하면 `isBackupDue` 호출을 지우면 된다 — 한 줄이다.

### 11.5 그리고 설정에서 그 문들을 닫았다

먼저 더하기만 하고 설정은 그대로 뒀는데 — 요청이 "이것들**도**"였으므로 — 그 결과
중복이 하나에서 셋이 됐다. 사용자가 "설정 탭에서 중복되는 건 없어졌어?"라고 물었고,
답은 "아니, 늘었다"였다. 그래서 이어서 지웠다.

| 지운 것 | 왜 |
|---|---|
| 계정 › **클라우드 데이터 새로고침** | 레일의 **아래쪽 절반**. §2.2가 그 절반만 하는 것이 왜 위험한지 적어뒀다 — 더 나쁜 판본으로 가는 두 번째 문이었다 |
| 계정 › **로컬 데이터 업로드** (버튼) | 이제 모든 동기화가 병합한다. 행은 남겼다 — §11.6 |
| 캘린더 › **전체 새로고침** | 레일이 누를 때마다 켜진 구독 전부를 돈다. 같은 호출로 가는 긴 길 |

| 남긴 것 | 왜 |
|---|---|
| 캘린더별 **"지금 새로고침"** | 하나를 고른다. 깨진 게 하나일 때 필요한 것은 그 하나다 |
| 데이터 › **"지금 백업"** | 레일은 예정일 때만 쓰고 이건 무조건이다(§11.4) — 다른 동작이다 |
| 계정의 `syncStatus` 문장과 `syncError` | 레일은 상태를 **아이콘**으로 그린다. 아이콘은 이유를 말하지 못한다 |

버튼을 지우면서 문자열 다섯 개(`auth.refreshCloud` · `auth.uploadLocal` ·
`auth.uploadSuccess` · `auth.uploadNoData` · `settings.calendar.refreshAll`)도
두 카탈로그에서 지웠다. `catalogue.test.ts`의 "모든 키는 어딘가에서 말해진다"가
정확히 그 다섯을 이름으로 불러 세웠다 — 화면을 지우면서 키를 두고 가는 일이
그 테스트가 막는 것이다.

`uploadLocalDataToSupabase`도 훅의 반환값에서 뺐다. 이제 `syncNow` 하나만 부른다.
`refreshSupabaseData`는 남는다 — `loadRace.test.tsx`가 로드 경합을 직접 몰기 위해
쓰는 손잡이다.

### 11.6 이주 업로드가 선택이 아니게 됐다 [관찰]

지금까지 이주 대기 데이터는 "업로드" 버튼을 누르거나, 누르지 않고 영영 두거나 둘 중
하나였다(거절 버튼이 없다). 이제 동기화를 누르면 병합된다. 합치기만 하고 지우지 않으므로
기록을 잃지는 않지만, "안 올리고 두기"라는 선택지는 사라졌다.

그래서 그 행을 지우지 않고 **문장만 남겼다.** 그 행은 버튼이자 알림이었고 — "로컬 항목
N개를 업로드할 수 있습니다"가 그런 것이 있다는 걸 알려주는 유일한 자리다 — 버튼이 없어도
알림으로서의 일은 남는다. 본문을 "다음 동기화 때 이 계정으로 합쳐집니다"로 바꿔서, 행이
이제 **약속**을 말하게 했다. `SettingsRow`의 `children`이 선택이 된 것은 이 한 행
때문이고, 비어 있는 컨트롤 상자는 그리지 않는다 — 자리만 차지한다.

### 11.7 무엇을 테스트했나

- `src/app/railSync.test.ts` — 9개. 그릴지 말지, 계정/구독의 상태가 어떻게 합쳐지는지.
- `e2e/railNotifications.spec.ts` — 둘 추가. 계정 없이 구독만 있을 때 버튼이 **그려지고**,
  누르면 **ICS를 실제로 다시 가져온다**. 헬퍼가 `lastSyncedAt`을 지금으로 심어서
  마운트의 자동 갱신(30분 기준)을 막으므로, 잡히는 요청은 버튼의 것이다.
  `syncAllExternalCalendars()` 호출을 지우면 이 테스트가 실패하는 것을 확인했다.
- 계정 쪽은 여전히 §10.3 그대로다 — E2E에 Supabase가 없다.
