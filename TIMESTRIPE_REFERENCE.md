...
| # | 할 일 | 왜 | 상태 |
|---|---|---|---|
| 1 | Horizons 페이지에서 만든 목표에 **보드를 지정**할 수 있게 | `projectId`가 안 붙어 **어느 Space에도 안 나타났다.** 두 축이 한 모델이라는 전제가 깨져 있었다 | **완료** 2026-08-14 |
| 2 | 목표의 **보드 변경** | 지평은 드래그로 오가는데 보드는 생성 시 고정. 축 하나가 write-once였다 | **완료** 2026-08-14 |
| 3 | Horizons 페이지에 **보드 이름 표시** | 색은 있는데 이름이 없어 어느 보드인지 그 화면에서 읽을 수 없었다 | **완료** 2026-08-14 |
| 4 | ~~보드 필터~~ | §4.2 — 하지 않는다 | 폐기 |

구현은 `SPACES_BOARD_DESIGN.md` Phase S3(D13~D15). 세 개가 배지 하나로 합쳐졌다 — 배지가 곧 보드 피커다.
