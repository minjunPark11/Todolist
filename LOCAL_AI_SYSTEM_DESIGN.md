...
2. **실행 옵션**: `-m <모델> --host 127.0.0.1 --port <탐색된 포트> --ctx-size 16384`.
   (시스템 프롬프트 + 앱 데이터 컨텍스트 + 지식 컨텍스트 + 대화 이력이 창을
   초과해 요청이 거부되는 문제로 4096 → 8192 → 16384로 상향. 한글은 문자당
   약 1토큰이라 문자 수 기반 상한(`AI_CONTEXT_LIMITS`)만으로는 부족해서,
   전송 직전에 `promptBudget.ts`가 토큰 추정 기반으로 프롬프트를 창 안에
   맞게 잘라낸다. 카탈로그 모델(Qwen2.5)은 모두 32k 학습 컨텍스트라 안전.)
