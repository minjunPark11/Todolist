...
2. **실행 옵션**: `-m <모델> --host 127.0.0.1 --port <탐색된 포트> --ctx-size 8192`.
   (시스템 프롬프트 + 앱 데이터 컨텍스트 + 지식 컨텍스트 + 대화 이력이 4096을
   초과해 요청이 거부되는 문제로 8192로 상향. 프롬프트 쪽도
   `AI_CONTEXT_LIMITS`로 상한을 둔다.)
