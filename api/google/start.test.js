...
function diff(a, b2, options, memorize = DEFAULT_MEMORIZE) {
  ...
  if (expectedType !== getType3(b2)) {
    ...
    aDisplay = memorize("expected", truncate3(aDisplay));
    bDisplay = memorize("actual", truncate3(bDisplay));
...
function comparePrimitive(a, b2, options, memorize = DEFAULT_MEMORIZE) {
  ...
  const bFormat = memorize("actual", format(b2, FORMAT_OPTIONS));
...
function getObjectsDifference(a, b2, formatOptions, options, memorize = DEFAULT_MEMORIZE) {
  ...
  if (aCompare === bCompare) {
    ...
    const bDisplay = memorize("actual", format(b2, formatOptions));
...
var onTestFailed = createTestHook("onTestFailed", (test5, handler2, timeout) => {
  test5.onFailed ||= [];
  test5.onFailed.push(withTimeout(handler2, timeout ?? getDefaultHookTimeout(), true, new Error("STACK_TRACE_ERROR"), abortIfTimeout));
});
var onTestFinished = createTestHook("onTestFinished", (test5, handler2, timeout) => {
  test5.onFinished ||= [];
  test5.onFinished.push(withTimeout(handler2, timeout ?? getDefaultHookTimeout(), true, new Error("STACK_TRACE_ERROR"), abortIfTimeout));
});
...
function createTestHook(name, handler2) {
  return (fn2, timeout) => {
    ...
    return handler2(current, fn2, timeout);
  };
}
...
function createSuiteCollector(name, factory = () => {
  ...
  const task = function(name2 = "", options = {}) {
    ...
    const handler2 = options.handler;
    if (task2.mode === "run" && !handler2) {
      task2.mode = "todo";
    }
    ...
    Error.stackTraceLimit = limit;
    if (handler2) {
      setFn(task2, withTimeout(withCancel(withAwaitAsyncAssertions(withFixtures(handler2, { context }), task2), task2.context.signal), timeout, false, stackTraceError, (_, error) => abortIfTimeout([context], error)));
    }
  ...
  const test5 = createTest(function(name2, optionsOrFn, timeoutOrTest) {
    let { options, handler: handler2 } = parseArguments(optionsOrFn, timeoutOrTest);
...
function createSuite() {
  ...
  suiteFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      const { options, handler: handler2 } = parseArguments(optionsOrFn, fnOrOptions);
      const fnFirst = typeof optionsOrFn === "function";
      cases.forEach((i2, idx) => {
        const items = Array.isArray(i2) ? i2 : [i2];
        if (fnFirst) {
          if (arrayOnlyCases) {
            suite2(formatTitle(_name, items, idx), handler2 ? () => handler2(...items) : void 0, options.timeout);
          } else {
            suite2(formatTitle(_name, items, idx), handler2 ? () => handler2(i2) : void 0, options.timeout);
          }
        } else {
          if (arrayOnlyCases) {
            suite2(formatTitle(_name, items, idx), options, handler2 ? () => handler2(...items) : void 0);
          } else {
            suite2(formatTitle(_name, items, idx), options, handler2 ? () => handler2(i2) : void 0);
          }
        }
      });
  ...
  suiteFn.for = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((item, idx) => {
        suite(formatTitle(name_, toArray(item), idx), options, handler2 ? () => handler2(item) : void 0);
      });
    };
  };
...
function createTaskCollector(fn2) {
  ...
  taskFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i2, idx) => {
        ...
        if (fnFirst) {
          if (arrayOnlyCases) {
            test5(formatTitle(_name, items, idx), handler2 ? () => handler2(...items) : void 0, options.timeout);
          } else {
            test5(formatTitle(_name, items, idx), handler2 ? () => handler2(i2) : void 0, options.timeout);
          }
        } else {
          if (arrayOnlyCases) {
            test5(formatTitle(_name, items, idx), options, handler2 ? () => handler2(...items) : void 0);
          } else {
            test5(formatTitle(_name, items, idx), options, handler2 ? () => handler2(i2) : void 0);
          }
        }
      });
      context.setContext("each", void 0);
  ...
  taskFn.for = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      const { options, handler: handler2 } = parseArguments(optionsOrFn, fnOrOptions);
      cases.forEach((item, idx) => {
        const handlerWrapper = handler2 ? (ctx) => handler2(item, ctx) : void 0;
        if (handlerWrapper) {
...
function createTestContext(test5, runner2) {
  ...
  context.onTestFailed = (handler2, timeout) => {
    test5.onFailed ||= [];
    test5.onFailed.push(withTimeout(handler2, timeout ?? runner2.config.hookTimeout, true, new Error("STACK_TRACE_ERROR"), (_, error) => abortController.abort(error)));
  };
  context.onTestFinished = (handler2, timeout) => {
    test5.onFinished ||= [];
    test5.onFinished.push(withTimeout(handler2, timeout ?? runner2.config.hookTimeout, true, new Error("STACK_TRACE_ERROR"), (_, error) => abortController.abort(error)));
  };
...
function stringDistanceCapped(strA, strB, cap) {
  ...
  for (let i2 = 0; i2 <= strA.length; i2++) {
    memo2[i2] = Array(strB.length + 1).fill(0);
    memo2[i2][0] = i2;
  }
        ...
        if (result && typeof result === "object" && typeof result.then === "function") {
          ...
          return thenable.then(({ pass: pass2, message: message2, actual: actual2, expected: expected2, meta: meta2 }) => {
            if (pass2 && isNot || !pass2 && !isNot) {
              const errorMessage = (customMessage ? `${customMessage}: ` : "") + message2();
...
function getCodeStartingAtIndex(code, index2, methodNames) {
  for (const name of methodNames) {
    const adjusted = index2 - name.length;
...
function replaceInlineSnap(code, s, currentIndex, newSnap, assertionName) {
  ...
  const { code: codeStartingAtIndex, index: index2 } = getCodeStartingAtIndex(code, currentIndex, methodNames);
...
var SnapshotState = class _SnapshotState {
  ...
  _inferInlineSnapshotStack(stacks) {
    ...
    const pollChainIndex = stacks.findIndex((i2) => i2.method.match(/__VITEST_POLL_CHAIN__/));
    if (pollChainIndex !== -1) {
    ...
    const customMatcherIndex = stacks.findIndex((i2) => i2.method.includes("__VITEST_EXTEND_ASSERTION__"));
...
${message}`);
  ...
  match({ testId, testName: testName2, received, key, inlineSnapshot, isInline, error, rawSnapshot, assertionName }) {
    const resolved = this._resolveKey(testId, testName2, key);
    key = resolved.key;
...
var SnapshotClient = class {
  ...
  async pollMatchDomain(options) {
    ...
    const timedOut = timeout > 0 ? new Promise((r) => setTimeout(r, timeout)) : void 0;
...
function createExpectPoll(expect2) {
  return function poll(fn2, options = {}) {
    ...
    chai_exports.util.flag(assertion, "_poll.fn", fn2);
    chai_exports.util.flag(assertion, "_poll.timeout", timeout);
    chai_exports.util.flag(assertion, "_poll.interval", interval);
    ...
    const proxy = new Proxy(assertion, { get(target, key, receiver) {
      ...
      return function __VITEST_POLL_CHAIN__(...args) {
        ...
        const promise = async () => {
          ...
          chai_exports.util.flag(assertion, "error", STACK_TRACE_ERROR);
...
function getAssertionName(assertion) {
  const name = chai_exports.util.flag(assertion, "_name");
...
function getTest(obj) {
  const test5 = chai_exports.util.flag(obj, "vitest-test");
...
function supabaseTokenVerifier(options) {
  ...
  async function loadKeys(force) {
    const age = cache ? now3().getTime() - cache.fetchedAt : Number.POSITIVE_INFINITY;
    if (cache && !force && age < cacheTtlMs) return cache;
    if (cache && force && age < refetchFloorMs) return cache;
    inFlight ??= fetchKeys(jwksUrl, fetchImpl, now3).finally(() => {
      inFlight = null;
    });
  ...
  return {
    async verify(bearer) {
      ...
      let keys2 = await loadKeys(false);
      let jwk = header.kid ? keys2.keys.get(header.kid) : void 0;
      if (!jwk) {
        keys2 = await loadKeys(true);
        jwk = header.kid ? keys2.keys.get(header.kid) : void 0;
      }
...
async function fetchKeys(url, fetchImpl, now3) {
  let response2;
  try {
    response2 = await fetchImpl(url, { headers: { Accept: "application/json" } });
  } catch {
    throw new UnauthorizedError("invalid_token", `The signing keys at ${url} could not be reached.`);
  }
  if (!response2.ok) {
    throw new UnauthorizedError("invalid_token", `The signing keys at ${url} came back ${response2.status}.`);
  }
  const body = await response2.json();
  const keys2 = /* @__PURE__ */ new Map();
  for (const key of body.keys ?? []) {
    if (key.kid) keys2.set(key.kid, key);
  }
  return { keys: keys2, fetchedAt: now3().getTime() };
}
...
var HOURS_AT_A_TIME_CHOICES = Array.from(
  ...
  (_, index2) => MIN_HOURS_AT_A_TIME + index2
);
...
var TABLE_TO_KEY = new Map(
  collectionTables.map(([key, table2]) => [table2, key])
);
...
function page(title, body) {
  const escape3 = (text) => text.replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" })[c]);
  return `<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>${escape3(title)}</title>
<style>
...
// src/functions/google/start.test.ts
function response() {
  const headers = {};
  return {
    headers,
...
var GOOD_STATE = "a1b2c3d4e5f60718.web";
var BROWSER = { accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8" };
var CURL = { accept: "*/*" };
beforeEach(() => {
...
describe("\uC5F0\uACB0 \uC2DC\uC791 \u2014 \uC124\uC815\uC774 \uC548 \uB41C \uBC30\uD3EC", () => {
  it("\uBE0C\uB77C\uC6B0\uC800\uC5D0\uB294 \uC77D\uC744 \uBB38\uC7A5\uACFC \uB3CC\uC544\uAC08 \uAE38\uC744 \uC900\uB2E4", () => {
    const res = response();
    handler({ method: "GET", query: { state: GOOD_STATE }, headers: BROWSER }, res);
    globalExpect(res.code).toBe(500);
    globalExpect(res.headers["content-type"], "JSON \uC73C\uB85C \uC8FC\uBA74 \uBE0C\uB77C\uC6B0\uC800\uAC00 \uB0A0 \uAC83\uC744 \uADF8\uB9B0\uB2E4").toContain("text/html");
    globalExpect(res.body).toContain("not set up on this server");
    globalExpect(res.body, "\uB3CC\uC544\uAC08 \uAE38\uC774 \uC5C6\uC73C\uBA74 \uC571\uC774 \uC0AC\uB77C\uC9C4 \uCC44\uB85C \uB05D\uB09C\uB2E4").toContain('href="/"');
    globalExpect(res.body, "\uB204\uB978 \uC0AC\uB78C\uC774 \uD560 \uC218 \uC788\uB294 \uC77C\uC774 \uC544\uB2C8\uB2E4 \u2014 \uBCC0\uC218 \uC774\uB984\uC740 \uC6B4\uC601\uC790\uC758 \uB2F5\uC774\uB2E4").not.toContain("GOOGLE_CLIENT_ID");
    globalExpect(res.payload, "\uBCF8\uBB38\uC744 \uB458\uB85C \uBCF4\uB0B4\uBA74 \uC548 \uB41C\uB2E4").toBeUndefined();
  });
  it("curl \uC5D0\uB294 \uC9C0\uAE08\uAE4C\uC9C0\uC640 \uB611\uAC19\uC740 JSON \uC744 \uC900\uB2E4 \u2014 \xA712.1 \uC758 \uD655\uC778 \uC808\uCC28", () => {
    const res = response();
    handler({ method: "GET", query: { state: GOOD_STATE }, headers: CURL }, res);
    globalExpect(res.code).toBe(500);
    const error = res.payload?.error ?? "";
    globalExpect(error, "\uBE60\uC9C4 \uBCC0\uC218 \uC774\uB984\uC774 \uBCF8\uBB38\uC5D0 \uC788\uC5B4\uC57C \uC124\uC815\uC774 \uB05D\uB0AC\uB294\uC9C0 \uD310\uC815\uD560 \uC218 \uC788\uB2E4").toContain("GOOGLE_CLIENT_ID");
    globalExpect(error).toContain("GOOGLE_CLIENT_SECRET");
    globalExpect(error).toContain("GOOGLE_REDIRECT_URI");
    globalExpect(res.body, "JSON \uC744 \uAE30\uB2E4\uB9AC\uB294 \uCABD\uC5D0 HTML \uC744 \uC8FC\uBA74 \uC548 \uB41C\uB2E4").toBe("");
  });
  it("Accept \uAC00 \uC544\uC608 \uC5C6\uC73C\uBA74 JSON \uC774\uB2E4 \u2014 \uAE30\uACC4\uAC00 \uAE30\uBCF8\uAC12\uC774\uB2E4", () => {
    const res = response();
    handler({ method: "GET", query: { state: GOOD_STATE } }, res);
    globalExpect(res.code).toBe(500);
    globalExpect(res.payload?.error).toContain("missing env");
  });
});
describe("\uC5F0\uACB0 \uC2DC\uC791 \u2014 \uC6B0\uB9AC\uAC00 \uC2DC\uC791\uD558\uC9C0 \uC54A\uC740 \uB9C1\uD06C", () => {
  it("\uBE0C\uB77C\uC6B0\uC800\uC5D0\uB294 \uBB34\uC5C7\uC744 \uD558\uB77C\uB294\uC9C0 \uB9D0\uD55C\uB2E4", () => {
    const res = response();
    handler({ method: "GET", query: { state: "\uC4F0\uB808\uAE30" }, headers: BROWSER }, res);
    globalExpect(res.code).toBe(400);
    globalExpect(res.headers["content-type"]).toContain("text/html");
    globalExpect(res.body).toContain("not one we started");
    globalExpect(res.body, "\uC5EC\uAE30\uC11C\uB3C4 \uB3CC\uC544\uAC08 \uAE38\uC740 \uC788\uC5B4\uC57C \uD55C\uB2E4").toContain('href="/"');
  });
  it("curl \uC5D0\uB294 JSON \uC774\uB2E4", () => {
    const res = response();
    handler({ method: "GET", query: { state: "\uC4F0\uB808\uAE30" }, headers: CURL }, res);
    globalExpect(res.code).toBe(400);
    globalExpect(res.payload?.error).toBe("Missing or malformed state.");
  });
});
describe("\uC790\uAE30 \uC810\uAC80", () => {
  it("\uC124\uC815\uC774 \uB05D\uB098 \uC788\uC73C\uBA74 \uAD6C\uAE00\uB85C \uBCF4\uB0B8\uB2E4 \u2014 \uBE0C\uB77C\uC6B0\uC800\uB4E0 \uC544\uB2C8\uB4E0", () => {
    ...
    for (const headers of [BROWSER, CURL]) {
      const res = response();
      handler({ method: "GET", query: { state: GOOD_STATE }, headers }, res);
      globalExpect(res.code).toBe(302);
      globalExpect(res.headers.location).toContain("accounts.google.com");
      globalExpect(res.headers.location, "state \uB294 \uADF8\uB300\uB85C \uC2E4\uB824 \uAC00\uC57C \uB3CC\uC544\uC62C \uB54C \uC6B0\uB9AC \uAC83\uC778\uC9C0 \uC548\uB2E4").toContain(encodeURIComponent(GOOD_STATE));
      globalExpect(res.headers["cache-control"], "\uD55C \uBC88 \uC4F0\uB294 state \uAC00 \uCE90\uC2DC\uB418\uBA74 \uB2E4\uC74C \uC2DC\uB3C4\uAC00 \uC78A\uD78C nonce \uB85C \uB3CC\uC544\uC628\uB2E4").toBe("no-store");
    }
  });
  it("GET \uC774 \uC544\uB2C8\uBA74 \uAC70\uC808\uD55C\uB2E4", () => {
    const res = response();
    handler({ method: "POST", query: { state: GOOD_STATE }, headers: BROWSER }, res);
    globalExpect(res.code).toBe(405);
    globalExpect(res.headers.allow).toBe("GET, HEAD");
  });
});
