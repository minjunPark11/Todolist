...
async function runReservedGoogleTaskOutbound(userId, operationId, deps) {
  ...
  const finish = async (outcome, source = null, agreed = null) => {
  ...
  try {
    ...
    const recurrence = begin.kind === "recurrence" ? taskRecurrence({ ...desired, ...object(begin.repeatTask) }, timezone) : void 0;
    ...
    const observed = object(await response.json());
    ...
    const source = object(await written.json());
...
async function runEventOperation(begin, operationId, deps, finish) {
  ...
  try {
    ...
    if (begin.kind === "create") {
      ...
      const source2 = object(await response2.json());
...
function createdByOperation(source, operationId) {
  try {
    return object(object(source.extendedProperties).private).focusflowOperation === operationId;
  } catch {
    return false;
  }
}
async function reconcileGoogleTaskOutbound(userId, operationId, deps) {
  try {
    ...
    let outcome = "applied", agreed = result?.kind === "agree" ? result.base : null;
