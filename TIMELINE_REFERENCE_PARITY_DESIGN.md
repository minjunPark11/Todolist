...
> **후속 [실측 · 2026-09-15]** — 기본 폭이 **200**으로 올라갔다. 144는 그대로
> 최소값이고 손잡이도 그대로이므로 이 절의 "둘 다"는 유지된다. 옮긴 이유는 앱을
> 띄워 재본 결과다: 144에서 트리의 행은 이름에 폴더 7px · 리스트 15px만 내주고,
> 두 글자짜리 "생활"이 "생."으로 나왔다. 들여쓰기(42) · 패딩(14) · gap(9)을 모두
> 깎아도 21px이라 들어가지 않는다 — 폭 말고 움직일 것이 없었다. 근거와 숫자는
> `src/app/contextSidebar.ts`에 있다(그 파일이 "폭을 정하는 곳은 여기다"라고
> 적어둔 자리).
...
P0부터 P7까지 전부 구현했다. 아래는 **설계와 다르게 끝난 것들**과, 구현 중에 드러나 설계가 몰랐던 사실들이다. 설계대로 된 것은 적지 않는다 — 그쪽은 코드와 §10의 표가 이미 말한다.

### 14.1 설계가 틀렸던 곳

**§9.6의 토스트는 만들 필요가 없었다.** `useTaskCommands`의 `mutate()`가 이미 모든 변경에 `notice`를 붙이고 `TaskUndoStrip`이 그것을 그린다. 설계가 "새 undo 스택을 만들지 않는다"고 적으면서도 표면은 새로 만들 것처럼 세어 뒀는데, 그 표면도 이미 있었다.

**§9.9의 툴팁은 `domain/floating`을 쓰지 않는다.** 재보니 비용이 안 맞는다 — 막대 하나가 이 줄무늬를 열두 개까지 들 수 있고, 떠 있는 표면은 포털에 자기 dismissal과 초점 규칙을 갖는데, 여기서 할 말은 상호작용이 필요 없는 짧은 세 줄이다. 네이티브 `title`이 길게 누르기에서도 살아남는다는 것은 덤이다.

**§9.5의 유령은 "끌고 있는 작업의 행"에 놓을 수 없다.** 레퍼런스는 미배치 작업에도 행을 주지만 이 앱은 날짜 없는 Item을 격자에서 아예 뺀다(D4·T-GV06). 가리킬 행이 없어서 유령은 포인터의 y를 따른다.

**§12.1의 세 등급은 두 등급이면 됐다.** 레퍼런스의 `bar-mid`가 하는 일은 총합만 숨기는 것인데, 우리 총합은 `margin-left: auto`로 이름에 밀려 있어 좁아지면 자기가 먼저 줄어든다. 남은 결정은 "언제 글자를 전부 버리나" 하나였고 그 값이 레퍼런스의 72px이다.

### 14.2 브라우저가 잡은 것 셋

**월 띠의 8px 패딩이 띠 경계를 규칙선에서 4.8px 밀고 있었다.** `box-sizing: border-box`에서 `flex-basis: 0`인 항목은 자기 패딩보다 좁아질 수 없어서, 8px이 비율보다 먼저 떼인다 — 각 띠가 `W × share`가 아니라 `8 + (W − 16) × share`가 됐다. 글자만 들여쓰는 것으로 고쳤다. 레퍼런스가 같은 자리에 `padding-left`를 쓰고도 멀쩡한 것은 세그먼트 폭을 `width: %`로 명시하기 때문이다. **`e2e/ganttBars`의 "띠 경계가 규칙선에 앉는다"가 이것을 잡았고, 그 테스트를 쓴 이유가 그것이었다.**

**오늘 칩이 `opacity`에 먹혀 월 띠가 비쳐 보였다.** 기둥에 `opacity: .55`를 걸면 자식까지 통째로 투명해지고, 칩에 `opacity: 1`을 적어도 되돌릴 수 없다. 색을 옅게 하는 것으로 바꿨다(`color-mix`). 칩이 월 띠를 뚫고 올라앉는다는 것이 이 배치의 전제였다.

**그림자 다섯 개가 §11.6에 걸렸다.** 목업은 전부 푸른기 도는 그림자를 쓰는데(`rgba(25,30,40,.06)` 따위) 이 앱에는 그림자에 색이 없어야 한다는 규칙이 있고 `e2e/surfaceHierarchy`가 화면의 모든 그림자에서 `r=g=b`를 확인한다. 기울기만 옮기고 색은 집의 `rgba(0,0,0,α)`로 맞췄다.

### 14.3 도메인이 잡은 것 하나

**진행 중 세션의 열린 구간은 `startAt`에서 시작한다.** `focusSessionStartOf()`(= `startedAt || startAt`)를 쓰면 10:30에 재개한 세션이 쉰 한 시간까지 일한 것으로 계산된다 — `engine.ts`의 `resume`이 러닝 세션의 `startAt`을 덮어쓰고 `startedAt`에 원래 시작을 남기기 때문이다. `focusTrace.test.ts`의 "resumes a running session after its last pause"가 이것을 잡았다.

### 14.4 §12의 위험, 실측 후

| | 설계가 적은 것 | 실제 |
|---|---|---|
| 1 | 막대 폭 등급 130/72가 우리 타이포에 맞는지 | 컨테이너 쿼리 유지, 두 등급으로 줄임(§14.1). 한글 이름에서도 72px 아래는 말줄임표뿐이라 값은 그대로 둔다 |
| 2 | 작업 열 264px에 네 칸이 들어가는가 | 점까지 넣어 고정분 124px, 이름에 140px. 900px 아래에서는 라벨 열을 190px로 올리고 **점과 날짜를 접는다** — 130px에 네 칸은 이름에 6px만 남긴다 |
| 3 | 사이드바 144와 저장된 232 | 그대로 남는다. 다만 **레퍼런스 둘이 이 숫자에서 어긋난다**는 것이 구현 중에 드러났다(§14.5) |
| 4 | 1초 타이머와 배터리 | `useFocusTick(false)`는 인터벌을 **만들지 않는다**. 아무것도 안 돌 때 화면은 트레이스가 생기기 전과 정확히 같다 |
| 5 | `showMilestones`의 쓸모 | 만들었고 기본 켜짐. 실제 데이터의 하루짜리 비율을 보고 재검토하는 것은 여전히 남는다 |

### 14.5 새로 드러난 충돌 한 건 — 사이드바 폭

설계 §4.6은 사이드바를 144px로 맞추기로 하면서 **목록 목업이 같은 열을 232px로 정해뒀다는 사실을 몰랐다**(`POLISHED_REFERENCE_PARITY_DESIGN.md` §2.1). 같은 앱의 두 목업이고 사이드바는 둘이 나눠 쓰는 한 열이라 둘 다 맞출 수 없다.

**타임라인 쪽을 택했다.** 그 화면이 가로를 가장 아쉬워하고, 사이드바에서 받은 88px이 곧 날짜다. 대가는 목록 화면에서 리스트 이름이 더 자주 잘리는 것이고 — 1440px 화면에서 `Paper`가 `Pap…`이 된다 [실측] — 그것이 이 결정의 알려진 비용이다. 손잡이와 저장은 그대로라 되돌리는 길은 손잡이 더블클릭이다.

이 줄은 **다시 열릴 수 있는 결정**으로 적어둔다: 기본값을 232로 되돌리고 바닥만 144로 두면 두 화면이 각자 자기 폭을 가질 수는 없지만, 목록을 쓰는 사람이 타임라인의 비율을 손으로 만들 수는 있다.

### 14.6 자에 더한 것

| 무엇 | 어디 | 왜 |
|---|---|---|
| 굵기 `500` · `560` | `REFERENCE.weight` | 눈금·본문이 500, 진행 중 집중 시간이 560. 목업의 620 한 곳은 650으로 스냅 |
| `.ff-timeline-focus-node` | `EXEMPT` | 1.8s/`ease-in-out`은 전환이 아니라 루프다. `.rail-sync.is-syncing`이 이미 있는 자리이고 이유도 같다 |
| `26-timeline.css` | `REFERENCE_FILES` | 한 줄이 한 번의 이행이다 |

크기·반경·간격·이징은 **한 칸도 더하지 않았다** — 설계 §6.2가 다섯 축을 대조한 결과가 맞았다.

### 14.7 검증

- `vitest run` — 3,655 통과 (설계 전 3,612에서 +43)
- `playwright test` — 데스크톱 206개 통과
- 새 도메인 테스트: `focusTrace.test.ts` 18개, `timeline.test.ts`의 `rulerBands`·`bandBoundaries`·`metaText`
- 지운 테스트: `todayColumn`(호출자가 없어졌다), `barText`/`barTextShort`(§4.1)

**체감은 여전히 검증 못 한다.** 헤드리스 창에서 `requestAnimationFrame`이 돌지 않아 숨쉬는 노드 · 트레이 밀림 · 유령 추적은 값이 맞는 것만 확인했다. GANTT §1이 적어둔 그대로이고, 사용자가 직접 봐야 한다.
